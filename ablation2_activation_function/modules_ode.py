...
class ODEFunc(nn.Module):
    ...
    non_linearity : string
        One of 'tanh' and 'softplus' ##updated to tanh from relu
    """
    def __init__(self, device, data_dim, hidden_dim, augment_dim=0,
                 time_dependent=False, non_linearity='tanh'): #updated to tanh from relu
        ...
        if non_linearity == 'tanh': # changed from relu to tanh
            self.non_linearity = nn.Tanh() # changed from nn.relu(inplace = True) to Tanh
...
class ODENet(nn.Module):
    ...
    def __init__(self, device, data_dim, hidden_dim, output_dim=1,
                 augment_dim=0, time_dependent=False, non_linearity='tanh',
                 tol=1e-3, adjoint=False): #updated to tanh from relu
