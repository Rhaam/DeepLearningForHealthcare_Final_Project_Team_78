'''
Updated: April-May 2023 by Team_78 for Final Project for the CS 598 Deep Learning in Healthcare class
'''
'''
...
class GRUOdeDecay(nn.Module):
  ...
  def __init__(self, input_size, hidden_size, bias=True):
    ...
    self.ode_net = ODENet(self.device, self.input_size, self.input_size, output_dim=self.input_size, augment_dim=0, time_dependent=False, non_linearity='tanh', tol=1e-3, adjoint=True)
  ##updated non_linearity from softplus to tanh throughout this file for ablation
...
if hp.net_variant == 'birnn_concat_time_delta':
  ...
  class Net(nn.Module):
    def __init__(self, num_static, num_dp_codes, num_cp_codes):
      ...
      # ODE layers
      self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
      self.ode_dp = ODENet(self.device, self.embed_dp_dim, self.embed_dp_dim, output_dim=self.embed_dp_dim, augment_dim=0, time_dependent=False, non_linearity='tanh', tol=1e-3, adjoint=True)
      self.ode_cp = ODENet(self.device, self.embed_cp_dim, self.embed_cp_dim, output_dim=self.embed_cp_dim, augment_dim=0, time_dependent=False, non_linearity='tanh', tol=1e-3, adjoint=True)
